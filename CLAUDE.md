...
With offline mode on, `/app` is not redirected to `/login` by `proxy.ts` — it renders its own access form (`AccessGate`), the single sign-in screen for both online and offline. Online it authenticates against Supabase and prepares/unlocks the local vault with the same password; offline it only unwraps the vault. `/api` still returns 401 without a session.
